angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  $scope.getSongData = function(artist){
    console.log(artist);
   itunesService.getArtists(artist).then(function(dataFromService){
      $scope.array = dataFromService;
      organize($scope.array);
    });
  };
  ...
  */
  ...
  function organize(arr) {
    for(var i = 0; i < arr.length; i++) {
      var tempObj = {};
      tempObj['AlbumArt'] = arr[i].artworkUrl100;
      tempObj['Artist'] = arr[i].artistName;
      tempObj['Collection'] = arr[i].collectionName;
      tempObj['CollectionPrice'] = '$' + arr[i].collectionPrice;
      tempObj['Play'] = arr[i].previewUrl;
      tempObj['Type'] = arr[i].kind;
      tempObj['Song'] = arr[i].trackName;
      newArr.push(tempObj);
    }
    return arr;
  }
